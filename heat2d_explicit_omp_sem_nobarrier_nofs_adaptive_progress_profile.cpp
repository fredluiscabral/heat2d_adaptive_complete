...
//
// Esta variante preserva a organizacao espacial da familia semaphore:
...
// //   - WAIT por semaforos POSIX em slots alinhados/padded.
//
...
// do backend, produzido por heat2d_wait_calibrate_semaphore.
//
...
#include "heat2d_progress_trace.hpp"
...
static inline void update_region_profiled(const Region& r, const double* src, double* dst,
                                          std::size_t ld, int N, int TILE, double mu,
                                          const double* north_halo, const double* south_halo,
                                          heat2d_progress::Trace& trace, int tid, int step) noexcept {
    const std::uint64_t total_points = r.ni<=0 ? 0ull :
        static_cast<std::uint64_t>(r.ni) * static_cast<std::uint64_t>(N - 2);
    heat2d_progress::TileMilestones milestones(trace, tid, step, total_points);
    if (r.ni<=0) { milestones.finish(); return; }
    ...
    for (int ii=r.first; ii<=r.last; ii+=TILE) {
        ...
        for (int jj=1; jj<=N-2; jj+=TILE) {
            ...
            milestones.advance(
                static_cast<std::uint64_t>(i_end - ii + 1) *
                static_cast<std::uint64_t>(j_end - jj + 1));
        }
    }
    milestones.finish();
}
...
int main() {
    ...
    heat2d_progress::Trace trace(ntmax, T);
    ...
    #pragma omp parallel default(shared)
    {
        ...
        for (int step=0;step<T;++step) {
            trace.mark(tid, step, heat2d_progress::STEP_START);
            const int min_level=step+1-cfg.max_lead;
            if (tid>0) {
                const bool w=wait_backend.wait_until_at_least(progress,tid,tid-1,min_level);
                if constexpr (PROFILE_STATS_ENABLED) if (w) ++st.lead_wait;
            }
            if (tid+1<nt) {
                const bool w=wait_backend.wait_until_at_least(progress,tid,tid+1,min_level);
                if constexpr (PROFILE_STATS_ENABLED) if (w) ++st.lead_wait;
            ...
            trace.mark(tid, step, heat2d_progress::DEPS_READY);
            ...
            update_region_profiled(rg,src,dst,ld,N,TILE,mu,rn.halo,rs.halo,trace,tid,step);
    ...
    trace.report(secs, "semaphore_adaptive");
    ...
    heat2d::print_summary("omp_semaforos_nofs_adaptive_progress_profile",p,dt,mu,secs,err);
