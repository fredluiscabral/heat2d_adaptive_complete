...
// Calibrador offline dos custos da resolucao adaptativa de dependencias.
// Executa a mesma aplicacao FTCS/OpenMP MPI-like e mede, em oportunidades reais,
// pares de custos de try-PREDICT (bound + budget + admissibilidade + construcao
// do halo predito) e RECOMPUTE. O resultado e gravado em heat2d_cost_model.dat
// para alimentar heat2d_explicit_omp_mpilike_adaptive_calibrated.cpp.
//
// Este executavel NAO deve ser usado para medir o tempo final da aplicacao: a
// instrumentacao e a recomputacao auxiliar fazem parte deliberadamente da
// calibracao.
//
...
int main() {
    ...
    // Modo de calibracao offline: mede pares P/R durante o bootstrap e depois
    // nao precisa reamostrar. O numero de amostras pode ser ajustado sem
    // recompilar; 16 por thread e um compromisso razoavel entre custo e robustez.
    cfg.enable_cost_model = true;
    cfg.cost_bootstrap_samples = getenv_int("HEAT2D_CALIBRATION_SAMPLES", 16);
    if (cfg.cost_bootstrap_samples < 2) cfg.cost_bootstrap_samples = 2;
    if (cfg.cost_bootstrap_samples > COST_BOOTSTRAP_MAX_SAMPLES)
        cfg.cost_bootstrap_samples = COST_BOOTSTRAP_MAX_SAMPLES;
    cfg.cost_resample_period = 0;

    print_config(cfg, mu);
    std::cout << "Calibration mode: offline\n"
              << "Calibration samples/thread: " << cfg.cost_bootstrap_samples << '\n';
    ...
    if (cfg.enable_cost_model) {
        ...
        std::ofstream cmodel(cost_file);
        if (!cmodel) {
            std::cerr << "Erro: nao foi possivel gravar " << cost_file << ".\n";
            return 4;
        }
        cmodel << std::setprecision(17)
               ...
               << "eta " << cfg.eta << '\n'
               << "kappa " << cfg.kappa << '\n'
               << "samples_per_thread " << cfg.cost_bootstrap_samples << '\n'
               << "ready_models " << ready_models << '\n'
               << "try_predict_ticks " << med_try << '\n'
               << "recompute_ticks " << med_rec << '\n'
               << "accept_probability " << med_acc << '\n';
        std::cout << "Calibration file: " << cost_file << '\n';
        if (ready_models < max_threads) {
            std::cout << "Calibration warning: only " << ready_models << " / "
                      << max_threads << " thread models reached the requested sample count.\n";
        }
    }
    ...
    heat2d::print_summary("omp_mpilike_calibrate", p, dt, mu, secs, err);
