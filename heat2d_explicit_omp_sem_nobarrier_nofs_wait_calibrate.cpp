// heat2d_explicit_omp_sem_nobarrier_nofs_wait_calibrate.cpp
// Offline calibration of the observed blocking cost for the semaphore Heat2D
...
static inline bool wait_for_level(const std::vector<ProgressSlot>& progress,
                                  std::vector<SemSlot>& notifications,
    ...
    while (progress[static_cast<std::size_t>(nb)].value.load(std::memory_order_acquire) < expected) {
        int rc;
        do {
            rc = sem_wait(&notifications[static_cast<std::size_t>(nb)].value);
...
int main() {
    ...
    #pragma omp parallel default(shared)
    {
        ...
        for (int step = 0; step < T; ++step) {
            if (tid > 0) {
                ...
                if (wait_for_level(progress, sem_right,  tid - 1, step, ticks)) ws.observe(ticks, wanted);
            }
            if (tid + 1 < nt) {
                std::uint64_t ticks = 0;
                if (wait_for_level(progress, sem_left,  tid + 1, step, ticks)) ws.observe(ticks, wanted);
            }
            ...
            sem_post(&sem_left[static_cast<std::size_t>(tid)].value); sem_post(&sem_right[static_cast<std::size_t>(tid)].value);
        }
    }
    ...
    const std::string path = (path_env && *path_env) ? path_env : "heat2d_wait_cost_semaphore.dat";
    ...
    std::cout << std::setprecision(16)
              << "Wait calibration backend: semaphore\n"
    ...
    heat2d::print_summary("wait_calibrate_semaphore", p, dt, mu, secs, err);
